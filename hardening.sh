...
# Add the configuration of location where rsyslog will store your logs
...
while true
do
  read -p "Digite a porta que deseja habilitar no firewall: " porta
  if [ $porta == 'x' ]; then
        break
  fi
...
echo "Allow port 514/tcp on firewall"
echo "Allow port 22/tcp on firewall"
...
echo "Configure the logging client"
...
echo "You can view the log messages on /var/log/remote ..."
...
aaaaaaaaaaaaa
  ...
  echo
  echo \*\*\*\* Install\ AIDE
  apt-get -y install aide  || dpkg -s aide &
